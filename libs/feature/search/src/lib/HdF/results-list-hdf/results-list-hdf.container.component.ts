...
import {
  RESULTS_HDF_LAYOUT_CONFIG,
  ResultsHdFLayoutConfigItem,
  ResultsHdFLayoutConfigModel,
  ResultsListHdFComponent,
...
export type ResultsListHdFShowMoreStrategy = 'auto' | 'button' | 'none'

@Component({
  selector: 'gn-ui-results-list-hdf-container',
  templateUrl: './results-list-hdf.container.component.html',
  styleUrls: ['./results-list-hdf.container.component.css'],
  standalone: true,
  imports: [
    CommonModule,
    ResultsListHdFComponent,
...
export class ResultsListHdFContainerComponent implements OnInit {
  @Input() metadataQualityDisplay: boolean
  @Input() layout: string
  @Input() showMore: ResultsListHdFShowMoreStrategy = 'auto'
  @Output() mdSelect = new EventEmitter<CatalogRecord>()

  layoutConfig$: Observable<ResultsHdFLayoutConfigItem>
  ...
  constructor(
    ...
    @Inject(RESULTS_HDF_LAYOUT_CONFIG)
    private resultsLayoutConfig: ResultsHdFLayoutConfigModel,
    @Optional()
