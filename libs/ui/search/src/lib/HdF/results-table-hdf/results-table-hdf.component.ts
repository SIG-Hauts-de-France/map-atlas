...
@Component({
  selector: 'gn-ui-results-table-hdf',
  templateUrl: './results-table-hdf.component.html',
  styleUrls: ['./results-table-hdf.component.css'],
  standalone: true,
  imports: [
    CommonModule,
    InteractiveTableComponent,
...
export class ResultsTableHdFComponent {
  ...
  getCardNumber(record: CatalogRecord): string {
    const resourceIdentifier = record.resourceIdentifier as
      | string
      | { code?: string }
      | undefined
    ...
    return resourceIdentifier.code ?? ''
  }
